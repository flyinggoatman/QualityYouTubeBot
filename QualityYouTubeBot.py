...
import openai1
from functions import channel_pull, video_pull, env_pull, about_pull, open_ai_func, Channels, insert_channel, check_channel_exists, delete_channel
import datetime
import pickle
import urllib.error
...
* http(s)://youtu.be/VIDEO_ID\r\r```
    ...
    try:
        ...
        if channel_url.startswith("!delete "):
            # Extract the URL provided with the delete command
            channel_url_to_delete = channel_url.split("!delete ")[1].strip()
            if re.search("/channel/", channel_url_to_delete) or re.search("@", channel_url_to_delete) or re.search("/user/", channel_url_to_delete) or re.search("/c/", channel_url_to_delete) or (not re.search("youtu.", channel_url_to_delete) and re.search("com/watch", channel_url_to_delete)):
                _, channel_id_link, _, channel_id = channel_pull(channel_url_to_delete, DEBUG_MODE)
            elif re.search("com/watch", channel_url_to_delete) or re.search("/shorts/", channel_url_to_delete) or re.search("youtu.be", channel_url_to_delete) or re.search(r"\?list=", channel_url_to_delete):
                _, channel_id_link, _, channel_id = video_pull(channel_url_to_delete, DEBUG_MODE)

            # Debug print to check if channel ID is correctly extracted
            print(f"Extracted channel ID for deletion: {channel_id}")

            # Use the extracted channel ID to delete the channel
            if await delete_channel(channel_id):
                ...
                print(f"Failed to delete the channel with ID: {channel_id}. It might not exist.")
        ...
        elif re.search("http://", channel_url) or re.search("https://", channel_url) and search("youtu", channel_url):
            ...
            elif re.search("com/watch", channel_url) or re.search("/shorts/", channel_url) or re.search("youtu.be", channel_url) or re.search(r"\?list=", channel_url):
            ...
            if re.search("UCMDQxm7cUx3yXkfeHa5zJIQ", channel_id_link):
                ...
                if AI_ON:
                    channel_description = await open_ai_func(OPENAI_API_KEY, channel_about, AI_ON, channel_name, message)
                ...
                if await check_channel_exists(channel_id):
                    ...
                    await message.channel.send(f"{channel_name}\r{channel_id_link}\rPosted by {author.display_name}", delete_after=num10)
